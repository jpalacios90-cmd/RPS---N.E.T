# Rock_Paper_Scissors---N.E.T
This code excutes a game of rock paper scissors.

# Here is a list of what to enter(detailed):
1. Your name
   a. Confirmation of if it's your name (y,n)
2. Enter the number of games you want to play.
3. You have these choices to enter:
4. a. 1) Rock
5. b. 2) Paper
6. c. 3) Scissors
7. d. Any other number besides 1, 2, or 3 will be invalid.
8. Enter any of the choices above.
9. a. The program enters a random choice.
10. Repeat step 3 until you've reach the number of games you entered in step 2.
11. A prompt will come up to ask wheter you'd like to continue
12. a. If 1(yes): Will restart to step 2
13. b. If 2(no): Game will end
14. Results will be stated when the game has ended.

# Here is a list of what to enter(simplified):
1. Enter your name
2. Enter the number of games you want to play
3. Enter 1(Rock), 2(Paper), or 3(Scissors)
4. Put 1 to continue game or 2 to end the game
5. Results are psoted at the end
